...
<template>
  <div 
    ref="cardRef"
    class="card bg-neutral-800 h-full hover:scale-[1.02] transition-transform duration-300 shadow-xl tv-focusable cursor-pointer"
    @click="showDetails"
    tabindex="0"
  >
...
onMounted(() => {
  if (cardRef.value) {
    const unregister = registerFocusable(cardRef.value, `movie-card-${props.movie.id}`)
